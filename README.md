...
| `wecomChat` | 各群记录看板：连接本机企业微信聊天记录解析服务（默认 `http://127.0.0.1:8767`，开放读取 API Key；`groups` 为可选群名白名单）。文件只走缓存；拆解任务与定位申报书见下文 |
...
- 管理后台「各群记录」：图片走企微 CDN 预览；文件卡从各电脑缓存下载（未在对应电脑打开过则下不了）；「定位申报书」按姓名/文件名/编号/时间窗对照已有任务；「拆解任务」识别申报书，并把附近修改意见（群聊文本或意见文档）一并上传，生成计划后仍须人工确认才写入
...
| `app/wecom_client.py` | 企业微信聊天记录解析服务只读客户端（合并多机会话、规范化文件消息、缓存取附件） |
| `app/wecom_locate.py` | 从群消息抽取姓名/文件名/编号/时间，对任务打分定位 |
| `app/wecom_cases.py` | 从群聊拆申报书，并收集附近修改意见（群文本 / 意见文档） |
...
| `app/routes/wecom_board.py` | 各群记录看板 API（管理员）：消息、缓存下载、定位任务、拆解上传 |
...
- 各群记录仅管理员；群文件只走企业微信本机/远端缓存，过期 CDN 链不当下载源
...
依赖本机企业微信解析服务（`wecomChat.baseUrl`，默认 `http://127.0.0.1:8767`）。同一群在多台电脑上合并为一条。

| 能力 | 说明 |
|------|------|
| 看板 | 按群看时间、发送人、正文、文件；自动刷新不打断下载/预览 |
| 下文件 | `POST /api/wecom/files` 按各电脑 `copies` 查缓存；哪台打开过就从哪台取。系统消息里的 `tpdownloadmedia` 链会收成文件卡，不当成正文 |
| 定位申报书 | 文件卡「定位申报书」→ `POST /api/wecom/locate-task`：用姓名 / 文件名 / 编号 / 时间窗（当天、7 天、30 天）对任务打分，打开 `/t/{id}`，下载该任务修改后/备份，以及群里那份原文件 |
| 拆解任务 | 聊天头「拆解任务」→ 规则拆解后调用现有 Gemini 协助定位申报书与修改意见（调用参数与分类相同，不改 llm / 密钥配置）→ 预览后上传。申报书须文件名含「申报书 / 申请表」或为带人名的 PDF；资料清单、聘用意向书、唯一申报承诺等不当申报书。修改意见可以是群聊文本，也可以是 Word / Excel / txt 等意见文档。未设日期时默认最近 14 天 |
| 人工确认 | 拆解上传后走现有两阶段：生成计划 → 首页/深链核对 → 「确认无误，写入文件」。缺缓存的申报书无法创建；意见文档未缓存时仍建任务，并尝试从标注栏抽意见 |
