...
function auditAppFiles(fileList) {
  ...
  fileList.forEach(function (f) {
    if (isAppFile(f.name)) {
      ...
      badNames.push(f.name + '（非 Word / Excel / PDF）');
    }
  });
...
function renderDetail(t) {
  ...
  var warnLine = t.applyWarning ? '<br>警告：<span style="color:#b26a00">' + esc(t.applyWarning) + '</span>' : '';
...
$('submitBtn').onclick = function () {
  ...
  if (!appFs.length) { errEl.textContent = '请选择申报书 Word / Excel 或 PDF'; return; }
...
function buildPlanEditor(el, t, plan) {
  ...
  el.querySelector('#addRowBtn').onclick = function () {
    var ne = { find: '', replace: '', clause: '（人工新增）', opinion: '', opinionGemini: '', opName: '', clauseId: '', section: '其他', appNo: appNo, findOk: true };
  ...
  el.querySelector('#applyBtn').onclick = function () {
    ...
    rows.forEach(function (tr) {
      ...
      var oi = parseInt(tr.getAttribute('data-oi'), 10);
      var src = curPlanData[oi] || {};
      if (src.findOk === false) preBad.push('#' + (oi + 1));
    });
    if (preBad.length === out.length && out.length) {
      errEl.textContent = '所选行的改前摘录均未在申报书中匹配，请先修正锚点或取消勾选';
      return;
    }
    if (preBad.length && !confirm('有 ' + preBad.length + ' 行改前摘录可能无法落盘（' + preBad.join('、') + '），仍要继续写入？')) return;
