...
def _explain(e: Exception, url: str, timeout_s: float) -> LlmError:
    ...
    if msg.startswith("HTTP 401") or "invalid token" in low or "incorrect api key" in low or "unauthorized" in low:
        return LlmError("API 密钥无效或未授权，请在管理后台检查 Gemini 配置")
    if msg.startswith("HTTP 429") or "rate limit" in low or "too many requests" in low:
        return LlmError("API 调用过于频繁或已达限额（HTTP 429），请稍后重试")
    if any(x in low for x in (
        "insufficient", "quota", "balance", "余额", "欠费", "credit", "billing",
        "exceeded your current quota", "resource_exhausted",
    )):
        return LlmError("API 余额不足或配额用尽，请充值后重试")
...
def _repair_json_commas(s: str) -> str:
    prev = None
    out = str(s or "")
    while prev != out:
        prev = out
...
def _salvage_clauses_array(s: str) -> dict | None:
    """分类 JSON 损坏时，尽量从 clauses 数组中捞出完整对象。"""
    text = str(s or "")
    m = re.search(r'"clauses"\s*:\s*\[', text, re.I)
    if not m:
        return None
    ...
    for i, ch in enumerate(arr_body):
        if ch == "{":
            ...
            if obj_depth == 0 and obj_start >= 0:
                chunk = arr_body[obj_start:i + 1]
                for cand in (chunk, _repair_json_commas(_escape_ctrl_in_json_strings(chunk))):
                    try:
                        obj = json.loads(cand)
                    except json.JSONDecodeError:
                        continue
...
def extract_json(text: str):
    return _decode_json_object(_strip_json_fence(text))


def extract_json_lenient(text: str):
    """比 extract_json 多一轮修复与 clauses 数组抢救，供分类/出计划使用。"""
    s = _strip_json_fence(text)
    last = None
    for cand in (s, _repair_json_commas(s), _escape_ctrl_in_json_strings(s), _repair_json_commas(_escape_ctrl_in_json_strings(s))):
        try:
            return _decode_json_object(cand)
        except json.JSONDecodeError as e:
            last = e
    salvaged = _salvage_clauses_array(s)
    if salvaged:
        return salvaged
    raise last or json.JSONDecodeError("JSON 解析失败", s or "", 0)
