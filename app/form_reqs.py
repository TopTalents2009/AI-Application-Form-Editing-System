...
def check_text_limits(app_text: str) -> list:
    ...
    for label, lim_title in (("项目成果", "项目成果"), ("简述个人贡献", "简述个人贡献")):
        ...
        lim = int(f["n"])
        pat = re.compile(
            re.escape(lim_title) + r"[^\n]{0,24}\n([^\n]+)",
            re.I,
        )
        for i, m in enumerate(pat.finditer(glued), 1):
            n = char_count(m.group(1))
            if n > lim:
                issues.append(
                    "第 %d 处「%s」现 %d 字，超过上限 %d 字。" % (i, lim_title, n, lim)
                )

    wp = by_title.get(("工作设想", "chars"))
    if wp:
        block = _slice_between(glued, r"工作设想", r"用人单位|承诺|附件|（8）|（9）")
        if not block.strip():
            block = _slice_between(glued, r"（7）工作设想", r"用人单位|承诺")
        n = _content_chars(block)
        if n > wp["n"]:
            issues.append(
                "「工作设想」现 %d 字，超过申报书印刷上限 %d 字（合格带 %d–%d 字），须压缩后再落盘。"
                % (n, wp["n"], _band(wp["n"])[0], _band(wp["n"])[1])
            )
    return issues
...
_SEC_FIELDS = {
    ...
    "工作": ("职务职责和贡献",),
    "项目": ("项目成果", "简述个人贡献"),
    "工作计划": ("申报人拟实现工作目标及可行性论证", "工作目标及可行性", "研发目标"),
    "工作设想": ("工作设想", "研发目标", "工作目标及可行性"),
    "专长成果": ("专长及代表性成果",),
...
def resolve_limit(find="", replace="", section="", app_text="") -> dict | None:
    ...
    if best is None and "引进企业基本情况" in blob_n:
        best = ent
    if best is None and re.search(r"工作设想|研发目标|技术路线|1500", blob_n):
        for cand in ("工作设想", "研发目标", "工作目标及可行性"):
        ...
        if best is None:
            glued = glue_limits(app_text)
            for f in char_fields:
                if "工作设想" in str(f.get("title") or "") or (
                    "1500" in glued and "设想" in str(f.get("title") or "")
                ):
                    best = f
                    break
    if best is None:
    ...
    for i, e in enumerate(edits or [], 1):
        ...
        for key, label in _TEXT_KEYS:
            ...
            if n_crop <= cap:
                ...
                continue
            compressed = compress_to_chars(cropped, cap, content_only=bool(lim.get("content")))
            n1 = _count_for_limit(compressed, lim)
            if n1 > cap:
                notes.append(
                    "第 %d 条「%s」replace 仍 %d 字，超过上限 %d 字，须人工精简后再确认落盘。"
                    % (i, lim["title"], n1, cap)
                )
                continue
            if n0 > cap and n1 < int(n0 * 0.55):
                notes.append(
                    "第 %d 条「%s」replace 由 %d 字压至 %d 字（压缩过半），请人工核对是否丢内容。"
                    % (i, lim["title"], n0, n1)
                )
            e[key] = compressed
